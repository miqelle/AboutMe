...
//function containing quiz questions 1-7
const aboutMeQuiz = function gameQuestions() {
  ...
  //for loop to give the user four try's
  for (let tries = 1; tries < 5; tries++) {
    let userAnswer = prompt("Guess a number between 1 and 5"); 
    if (userAnswer > 4) {
  ...
  for (
    i = 1;
    i < 7;
    i++ //this for loop asks the user the question 6 times, and then tells them all possible answers
  ) {
    let userAnswer = prompt("What city would I like to visit?");
    if (userAnswer === citiesToVisit[0]) {
      rightAnswers++;
      alert(
        `That's the number one city I want to visit! You got  ${rightAnswers}  right answers out of 7 😊`
      );
      break;
    }
    if (userAnswer !== citiesToVisit[0]) {
      alert(
        `All possible answers: New Orleans, St. Louis, San Diego, San Francisco  You got  ${rightAnswers}  right answers out of 7`
      );
    }
  }
};
